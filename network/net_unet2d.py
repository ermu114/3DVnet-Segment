...
class UNet2D(object):
    ...
    def up_and_concate(self, down_layer, layer):
        in_channel = down_layer.get_shape().as_list()[3]
        out_channel = in_channel // 2
        up_layer = Conv2DTranspose(out_channel, [2, 2], strides=[2, 2])(down_layer)
        print("--------------")
        print(str(up_layer.get_shape()))
        print(str(layer.get_shape()))
        print("--------------")
        my_concat = Lambda(lambda x: K.concatenate([x[0], x[1]], axis=3))
        concate = my_concat([up_layer, layer])
        # must use lambda
        # concate=K.concatenate([up, layer], 3)
        return concate

    def attention_block_2d(self, x, g, inter_channel):
        '''
    ...
    def res_block_v2(self, input_layer, out_n_filters, batch_normalization=False, kernel_size=[3, 3], stride=[1, 1],
                     padding='same'):
    ...
    def unet_model_2d_attention(self, input_shape, n_labels, batch_normalization=False, initial_learning_rate=0.00001,
        ...
        # down_layer_1
        layer = self.res_block_v2(layer, 64, batch_normalization=batch_normalization)
        ...
        # down_layer_2
        layer = self.res_block_v2(layer, 128, batch_normalization=batch_normalization)
        down_layer.append(layer)
        layer = MaxPooling2D(pool_size=[2, 2], strides=[2, 2])(layer)
        ...
        # down_layer_3
        layer = self.res_block_v2(layer, 256, batch_normalization=batch_normalization)
        down_layer.append(layer)
        layer = MaxPooling2D(pool_size=[2, 2], strides=[2, 2])(layer)
        ...
        # down_layer_4
        layer = self.res_block_v2(layer, 512, batch_normalization=batch_normalization)
        down_layer.append(layer)
        layer = MaxPooling2D(pool_size=[2, 2], strides=[2, 2])(layer)
        ...
        # bottle_layer
        layer = self.res_block_v2(layer, 1024, batch_normalization=batch_normalization)

        print(str(layer.get_shape()))

        # up_layer_4
        layer = self.attention_block_2d(down_layer[3], layer, 256)
        layer = self.res_block_v2(layer, 512, batch_normalization=batch_normalization)

        print(str(layer.get_shape()))

        # up_layer_3
        layer = self.attention_block_2d(down_layer[2], layer, 128)
        layer = self.res_block_v2(layer, 256, batch_normalization=batch_normalization)
        print(str(layer.get_shape()))

        # up_layer_2
        layer = self.attention_block_2d(down_layer[1], layer, 64)
        layer = self.res_block_v2(layer, 128, batch_normalization=batch_normalization)
        print(str(layer.get_shape()))

        # up_layer_1
        layer = self.attention_block_2d(down_layer[0], layer, 32)
        layer = self.res_block_v2(layer, 64, batch_normalization=batch_normalization)
        print(str(layer.get_shape()))
    ...
    def unet_model_2d(self, input_shape, n_labels, batch_normalization=False, initial_learning_rate=0.00001,
                      metrics=m.dice_coefficient):
        """
        input_shape:without batch_size,(img_height,img_width,img_depth)
        metrics:
        ...
        # up_layer_4
        layer = self.up_and_concate(layer, down_layer[3])
        layer = self.res_block_v2(layer, 512, batch_normalization=batch_normalization)
        print(str(layer.get_shape()))

        # up_layer_3
        layer = self.up_and_concate(layer, down_layer[2])
        layer = self.res_block_v2(layer, 256, batch_normalization=batch_normalization)
        print(str(layer.get_shape()))

        # up_layer_2
        layer = self.up_and_concate(layer, down_layer[1])
        layer = self.res_block_v2(layer, 128, batch_normalization=batch_normalization)
        print(str(layer.get_shape()))

        # up_layer_1
        layer = self.up_and_concate(layer, down_layer[0])
        layer = self.res_block_v2(layer, 64, batch_normalization=batch_normalization)
        print(str(layer.get_shape()))
